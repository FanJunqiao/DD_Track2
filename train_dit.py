...
from data import ImageFolder, transform_cifar
...
def main(args):
    ...
    if args.spec == "imagenet_tiny":
        ...
        dataset = ImageFolder(args.data_path, transform=transform, nclass=args.nclass,
                            ...
                            seed=0, return_origin=args.condense)
    elif args.spec == "cifar100":
        train_transform, test_transform = transform_cifar(augment=False, from_tensor=False, normalize=False)
        dataset = datasets.CIFAR100("./dataset", train=True,
    ...
    for epoch in range(args.epochs):
        ...
        for databatch in tqdm(loader, total=len(loader), desc='Train round', unit='batch', leave=False):
            if args.condense:
                x, ry, y = databatch
                ry = ry.numpy()
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--lambda-neg', default=0.001, type=float, help='weight for diversity constraint')
